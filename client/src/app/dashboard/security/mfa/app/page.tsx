/**
 * Authenticator App 设置页面 - /dashboard/security/mfa/app
 * 使用真实 API 启用 TOTP 2FA
 */
...
export default function MfaAppPage() {
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast(err.response?.data?.error || err.message || '密码验证失败');
    }
  };
  ...
  const handleVerifySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast(err.response?.data?.error || err.message || '验证码错误');
    }
  };
          ...
          <form onSubmit={handlePasswordSubmit} className="max-w-md mx-auto">
            <div className="flex flex-col items-center mb-8">
              <div className="text-center space-y-2">
                <h3 className="text-lg font-medium text-regular">验证密码</h3>
            ...
            <div className="flex flex-col items-center mb-10">
              ...
              {setup2FAData?.secret && (
                <div className="mt-4 text-center">
                  <p className="text-xs text-muted mb-1">或手动输入密钥：</p>
